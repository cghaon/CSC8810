...
class WGAN(object):
    ...
    def discriminator(self, x, is_training=True, reuse=False):
        # Network Architecture is inspired by (https://arxiv.org/abs/1606.03657)
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
          #64,128,256,512
            ...
            net = lrelu(conv2d(net, 512, 4, 4, 4, 4, name='d_conv4'))
    ...
    def generator(self, z, is_training=True, reuse=False):
        # Network Architecture is inspired by infoGAN (https://arxiv.org/abs/1606.03657)
        # Architecture : FC1024_BR-FC7x7x128_BR-(64)4dc2s_BR-(1)4dc2s_S
        with tf.variable_scope("generator", reuse=reuse):
            net = tf.nn.relu(bn(linear(z, 2*2*512, scope='g_fc1'), is_training=is_training, scope='g_bn1'))
            net = tf.reshape(net, [self.batch_size, 2, 2, 512])
    ...
    def build_model(self):
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.d_optim = tf.train.AdamOptimizer(self.learning_rate_Q/2, beta1=self.beta1) \
                      .minimize(self.d_loss, var_list=d_vars)
            self.g_optim = tf.train.AdamOptimizer(self.learning_rate_Q/2, beta1=self.beta1) \
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in range(start_batch_id, self.num_batches):
                ...
                # save training results for every 400 steps
        ...
        plt.plot(x, IS) #plot IS vs epoch
        plt.xlabel('epoch') #label X
        plt.ylabel("IS") #label Y
